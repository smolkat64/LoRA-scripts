...
# Настройки обучения
$unet_lr = 1e-3 # Скорость обучения U-Net
$text_encoder_lr = 1e-3 # Скорость обучения текстового энкодера
...
if ($is_structure_wrong -eq 0 -and $abort_script -ne "y")
{
	...
	# advanced
	$run_parameters += " --unet_lr=$unet_lr --text_encoder_lr=$text_encoder_lr"
